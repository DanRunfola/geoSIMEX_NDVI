...
First, we use a monte carlo simulation procedure to illustrate the relative accuracy of geoSIMEX as contrasted to other procedures.  
...
In practice, combinations of different levels of precision in the documentation of individual aid projects result in $lambda$ values between these two extremes, providing a single measurement of spatial imprecision for any set of spatially imprecise measurements.
\par
...
For each unit of observation (i.e., district), the total dollars of aid is estimated by examining the overlap between the possible region aid could be in and the unit of observation (i.e., see table \ref{precision_example}).
